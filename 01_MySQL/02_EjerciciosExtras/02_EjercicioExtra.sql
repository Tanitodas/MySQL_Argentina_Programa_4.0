...
-- o Utilizando la función DATE_FORMAT de MySQL.
	...
	-- Si utilizamos "%y" nos toma los DOS ultimos digitos del año
	SELECT codigo_cliente , fecha_pago FROM pago
    WHERE date_format(fecha_pago,'%y') = '08';
    
     -- Si utilizamos "%Y" nos toma los CUATRO digitos del año
    SELECT codigo_cliente , fecha_pago FROM pago
...
-- o Utilizando la función ADDDATE de MySQL.
    ...
    /* La función ADDDATE se utiliza para restar 2 días (-2) a la fecha esperada
       y luego se compara con la fecha de entrega en la cláusula WHERE. */
...
-- o Utilizando la función DATEDIFF de MySQL.
    ...
    /*   La función DATEDIFF se utiliza para calcular la diferencia en días entre la fecha de entrega
         y la fecha esperada. Luego, se compara si esa diferencia es mayor o igual a 2 en la cláusula WHERE. */
...
-- cualquier año.
    ...
    /* Funcion MONTH() nos devuelve el mes de cualquier fecha proporcionada
	   como argumento a dicho funcuion */
...
-- venta, mostrando en primer lugar los de mayor precio.
	...
	SELECT gama , cantidad_en_stock AS Stock , precio_venta
    FROM producto
...
-- representante de ventas tenga el código de empleado 11 o 30.
	...
	SELECT nombre_cliente , codigo_empleado_rep_ventas , ciudad
    FROM cliente
...
-- representantes de ventas.
	...
	SELECT c.nombre_cliente AS Clintes_Que_Realizaron_Pagos , e.nombre AS NombreRep , e.apellido1 AS Apellido1Rep , e.apellido2 AS Apellido2Rep
    FROM cliente c
...
-- junto con la ciudad de la oficina a la que pertenece el representante.
	...
	SELECT c.nombre_cliente AS Clintes_Que_Realizaron_Pagos , e.nombre AS NombreRep ,
    e.apellido1 AS Apellido1Rep , e.apellido2 AS Apellido2Rep , o.ciudad AS CiudadRep
    FROM cliente c
    INNER JOIN empleado e ON c.codigo_empleado_rep_ventas = e.codigo_empleado
...
-- representantes junto con la ciudad de la oficina a la que pertenece el representante.
	...
	SELECT c.nombre_cliente AS Clintes_Que_NO_Realizaron_Pagos , e.nombre AS NombreRep ,
    e.apellido1 AS Apellido1Rep , e.apellido2 AS Apellido2Rep , o.ciudad AS CiudadRep
    FROM cliente c
    INNER JOIN empleado e ON c.codigo_empleado_rep_ventas = e.codigo_empleado
	INNER JOIN oficina o ON o.codigo_oficina = e.codigo_oficina
...
-- 6. Lista la dirección de las oficinas que tengan clientes en Fuenlabrada.
	...
	SELECT c.ciudad AS CiudadCliente, c.nombre_cliente AS NombreCliente, o.linea_direccion1 AS DireccionOficina1 ,
    o.linea_direccion2 AS DireccionOficina2 FROM oficina o
...
-- de la oficina a la que pertenece el representante.
	...
	SELECT c.nombre_cliente ,  e.nombre AS NombreRep , e.apellido1 AS Apellido1Rep , e.apellido2 AS Apellido2Rep ,
    o.ciudad AS NombreCiudadOficina FROM oficina o
    INNER JOIN empleado e ON e.codigo_oficina = o.codigo_oficina
    INNER JOIN cliente c ON c.codigo_empleado_rep_ventas = e.codigo_empleado
    GROUP BY e.nombre , e.apellido1 , e.apellido2 , c.nombre_cliente , o.ciudad ;

-- 8. Devuelve un listado con el nombre de los empleados junto con el nombre de sus jefes.

	SELECT e1.nombre AS NombreEMP , e2.nombre AS NombreJEFE
    FROM empleado e1
...
-- 10. Devuelve un listado de las diferentes gamas de producto que ha comprado cada cliente.
	...
	SELECT c.nombre_cliente , pro.gama AS Gama_Producto 
    FROM cliente c
    ...
    GROUP BY c.nombre_cliente , gama_producto;
...
-- no han realizado ningún pedido.
    ...
    WHERE p.codigo_cliente IS NULL AND ped.codigo_cliente IS NULL
    GROUP BY c.nombre_cliente;

-- 4. Devuelve un listado que muestre solamente los empleados que no tienen una oficina
-- asociada.

	SELECT e.nombre AS NombreEMP
    FROM empleado e
...
-- asociado.
	...
	SELECT e.nombre AS NombreEMP , e.codigo_empleado
    FROM empleado e
...
-- de la gama Frutales
	...
	SELECT o.linea_direccion1 AS Direccion1 , o.linea_direccion2 AS Direccion2 , e.nombre AS NombreEmpVentas , c.nombre_cliente AS NombreCliente
    FROM oficina o
    ...
    LEFT JOIN producto prod ON prod.codigo_producto = dp.codigo_producto
    WHERE prod.gama = 'frutales'
    GROUP BY Direccion1 , Direccion2 , NombreEmpVentas , NombreCliente;
...
-- ningún pago.
    ...
    FROM cliente c
    LEFT JOIN pedido p ON p.codigo_cliente = c.codigo_cliente
    LEFT JOIN pago ON pago.codigo_cliente = c.codigo_cliente
...
-- nombre de su jefe asociado.
	...
	SELECT e1.nombre AS NombreEMP , e1.codigo_empleado AS CodigoEMP , e2.nombre AS NombreJEFE
    FROM empleado e1
    INNER JOIN empleado e2 ON e1.codigo_jefe = e2.codigo_empleado
...
-- 5. Calcula el precio de venta del producto más caro y más barato en una misma consulta.
	...
	SELECT max(precio_venta) AS PrecioMaximoDeVent , min(precio_venta) AS PrecioMinimoDeVent
    FROM producto;
...
-- cada uno.
	...
	SELECT e.nombre AS NombreEMP , e.apellido1 AS Apellido1 , e.apellido2 AS Apellido2 ,
    count(c.codigo_empleado_rep_ventas) AS CantidadDeClientesPorEmpleado
    FROM empleado e
    INNER JOIN cliente c ON c.codigo_empleado_rep_ventas = e.codigo_empleado
    GROUP BY NombreEMP , Apellido1 , Apellido2;
...
-- deberá mostrar el nombre y los apellidos de cada cliente.
	...
	SELECT c.nombre_cliente AS NombreCliente , p.fecha_pago AS PrimerFechaPago , p.fecha_pago AS UltimaFechaPago
    FROM cliente c 
    LEFT JOIN pedido p ON p.codigo_cliente = c.codigo_cliente
...
-- 12. Calcula el número de productos diferentes que hay en cada uno de los pedidos.
	...
	SELECT count(dp.codigo_producto) AS NumeroDiferentesDeProd , p.codigo_pedido
    FROM pedido p
    RIGHT JOIN detalle_pedido dp ON dp.codigo_pedido = p.codigo_pedido
...
-- los pedidos.
	...
	SELECT sum(dp.cantidad) AS CantidadDeProductosPorPedido , p.codigo_pedido
    FROM pedido p
    RIGHT JOIN detalle_pedido dp ON dp.codigo_pedido = p.codigo_pedido
    GROUP BY p.codigo_pedido
    ORDER BY p.codigo_pedido ASC;
...
-- unidades vendidas.
	...
	SELECT prod.nombre AS NombreProducto , prod.codigo_producto , sum(dp.cantidad) AS CantidadDeUnidadesVendidas
    FROM producto prod
    RIGHT JOIN detalle_pedido dp ON dp.codigo_producto = prod.codigo_producto
    GROUP BY NombreProducto , prod.codigo_producto
...
-- imponible, y el total la suma de los dos campos anteriores.
	...
	SELECT sum(pago.total) AS FacturacionTotal , sum(dp.cantidad 
